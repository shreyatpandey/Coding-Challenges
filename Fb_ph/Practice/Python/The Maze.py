...
Complexity:-
Tc:- O(m.n.(m+n))
For BFS operation each queue take O(1) time, and a single node
can be pushed once leading to O(m.n) operations
For the directions loop, we would take O(m) steps for vertical directions or
O(n) steps for horizontal directions to reach a wall.
Sc:- O(m.n)
** visited set ** = O(m.n)
** BFS queue ** = O(m.n)
'''
